...
class TickHandler:
    ...
    def __init__(self, agent_version):
        ...
        for si in self.config.symbol_info:
            ...
            if resp.status_code != 200:
                logger.warning(f"Connection Error: http://127.0.0.1:2000")
                start_mt5_api(si.broker, si.symbol)
                resp = requests.get(f"http://127.0.0.1:2000/healthcheck")
                assert (
                    resp.stats_code == 200
                ), f"http://127.0.0.1:2000 failed to initialize"
            mt5_config = mt5_creds[si.broker][self.config.raw_data.data_mode]
            _ = requests.post(f"http://127.0.0.1:2000/init", json=mt5_config)
    ...
    def init_tick_data(self, dt1, hour_delta=72):
        ...
        args = []
        for si in self.config.symbol_info:
            args.append((si.broker, si.symbol, dt0, dt1, self.config.raw_data.data_mode, False))
